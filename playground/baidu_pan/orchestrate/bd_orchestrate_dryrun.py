"""百度网盘转存编排原型（只读 dry-run，不做任何转存）。【已升包，留档】
选点/对比/补齐逻辑已搬至 scrape_all/sites/baidu_pan/orchestrate.py（含单测
tests/test_baidu_pan_orchestrate.py），dry-run 正式入口 scripts/consume_posts.py。2026-08-24。
...
用法：
  python playground/baidu_pan/orchestrate/bd_orchestrate_dryrun.py --ids 225896,216571   # 指定帖子 id
  python playground/baidu_pan/orchestrate/bd_orchestrate_dryrun.py --limit 3             # 最新 3 个帖子
  python playground/baidu_pan/orchestrate/bd_orchestrate_dryrun.py --test                # config.TEST_LINKS
  python playground/baidu_pan/orchestrate/bd_orchestrate_dryrun.py                       # 全部（35 个左右，慢）
