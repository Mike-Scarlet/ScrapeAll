...
- **已接入并真链接验证**：
  ...
  - **流媒体源站 track**（SOURCE_HOSTS kind=source，页面流，需 stealth 会话 `DownloadEngine(stealth=True)`）：`hanime`（首家，download 页表格选最高画质，attachment 直链与页内 fetch→blob 双路径，细节见 adapter 头注释）、`rule34video`（KVS 架构，2026-08-30 收口）：视频页服务端渲染下载区 `a.tag_item_download` 每档一条，href = `/get_file/…?v-acctoken=…&download=true&download_filename=真名`（**token 与 PHPSESSID 绑定、每次渲染新发，绝不跨会话缓存**，每次重开视频页读新鲜链接）；锚文本解析分辨率取最高、**上限 1080p**（2026-08-30 从 2160p 调低省流量；全部超上限/认不出保页面原序首行兜底，档位进 note 可见）；点击即 attachment 直链，浏览器下载器流式落盘（4K 档实测 449.2MB，绝不能走 blob）；get_file 302 跳跨域 CDN（boomio-cdn.com，**实例会轮换**），视频页内 fetch 被 CORS 掐——体积探活 = 点击收下载事件拿最终直链 → 立刻 cancel → 在最终 origin 停页同源 Range:0-0（0 正文流量）。**登录墙形态**（311026 实案）：http 200 但无 `<video>` 元素 + 登录字样 → probe=needs_auth / download=manual（个别视频登录后才可见；占位容器 class 带 "player" 字样会误伤子串选择器，判据只认 `<video>` 标签数）。验证（2026-08-30）：probe alive（5 档选 2160p、真名+449.2MB+cdn 206）＋ download 事件级（monkeypatch `engine.save_download` 收口：元信息打印后事件即 cancel 落哑文件，大件合规）＋ 幂等复跑 skipped 零流量；单测 29 例（档位/上限/真名解析/注册表），全量 348 passed。**放量实录（2026-08-30）**：22 帖全收口——23 下载 4003MB / 5 死链（404） / 2 exhausted（members 主页非视频形态 + 登录墙当时误 unknown 耗尽，后修正分类翻 manual 转人工）。重开配方（adapter 后补接入存量时复用）：EroLink 批量 `set_link_status(url, pending)` + 所在帖 stat 3→2（与 mark_consumed 对称的 RecordFieldChanged），consume 下轮即吃。坑两条：**`context.on("page")` 对 `new_page()` 开的页也触发**——当广告弹窗杀手用会把自家主页 0.1s 内关掉（goto 全挂的元凶），别用；KVS 直链路径带尾斜杠，取 basename 前先 rstrip、`hmvmania`（WordPress + viewtube 主题，2026-09-01 收口，**最简一家：零 API 零 token 零跨域，普通会话即可 stealth 不必须**）：视频页服务端渲染 `ul.video-meta` 下载项 `<li><i class="fas fa-download"></i><a href="…/wp-content/uploads/…mp4" download>DL</a></li>`，锚点静态渲染（curl 裸拉就有）；选择器 `a[download][href^='http']` 框在 video-meta 里——播放器 JS 模板另有条 `<a href="{{ data.url }}" download>`，href 非 http 前缀被排除，rows JS 与点击 nth 同用属性值语义索引严格对齐。**一视频一文件**（12 页抽查全单档），真名前缀即规格 `av1_1080p_真名.mp4`（AV1 编码+分辨率，个别无标记如 Zen-AGEPLAY.mp4）；锚文本恒为 "DL" 没信息量，分辨率从 href basename 解析，多档保险按 ≤1080p 上限取最高（与 rule34 同策）。点击即下载事件（同源直链 + download 属性，无 attachment 头也强制落盘）；**体积探活 = 视频页内同源 Range:0-0**（文件就在 hmvmania.com origin，CF 边缘缓存 206 content-range，不用 rule34 的停页把戏），probe 全程 0 正文流量 0 点击。状态判定：404/410 dead / CF 标题 unknown / 其余 unknown-failed；会员墙形态未标定（页面登录字样均导航装饰），保守不判 needs_auth。非 `/video/<slug>/` 形态（/author/、分类页、wp-content 直链）parse 直接挡。验证（2026-09-01）：321390 (CC005 Beethoven) probe alive（真名+53.7MB+range 206）→ download 落盘 56,314,637B 与探活总长逐字节一致，ffprobe av1 1920x1080 + opus、时长 226.2s 与页面标 3:46 吻合 → 幂等复跑 skipped 零流量；存量 8 条 EroLink 全量 probe 8/8 alive（17.3–121.3MB，合计约 560MB）；单测 25 例，全量 415 passed。**放量实录（2026-09-01）**：重开配方走起（8 链接 set pending + 8 帖 stat 3→2），consume --ids 定向 8 帖——**8 下载 559.5MB（586,700,758B）/ 0 死链 / 0 异常 / 8/8 帖收口回 CONSUMED**；终验盘上字节与 EroLink dl_size 逐条一致，ffprobe 抽查全 av1 1920x1080 + opus。319931 特记：人工补位时期的 "Short stroke Smooth 变体"（46,151,757B）与新落本尊 av1_1080p 同字节——该站一视频一文件，变体实为本尊改名，两件并存无覆盖
